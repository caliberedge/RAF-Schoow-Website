...
<?php
		...
		//Create INSERT query
$qry2 = "INSERT INTO webpay(date, dpurpose, damount, email, pincode, slipno, school, dname, dphoneno, dchild, dpay, acad_session, p_status, tranz_stat) VALUES(now(), '$purpose', '$damount', '$email', '$pincode', '$slipno', '$school', '$sdsurname', '$phone', '$sdothernames', '$dpay','2021/2022','not used','nil')";
...
$qry3="SELECT * FROM transformx WHERE pincode='$pincode' AND slipno='$slipno'";
	...
	if(mysql_num_rows($result3) > 0) {
		//	redirect script to payment gateway
			...
			session_write_close();
			//header("location: pat_transform.php?gix=$url");
			header("location: tranx_paystk.php?inv=$pin&cd=$url");
			exit();
			exit();   
  } else {

		header("location: txtcform_test.php");
}
}
}
}
